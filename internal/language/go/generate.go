...
func (gl *goLang) GenerateRules(c *config.Config, dir, rel string, f *rule.File, subdirs, regularFiles, genFiles []string, other []*rule.Rule) (empty, gen []*rule.Rule) {
	...
	pkg := buildPackage(c, dir, rel, pkgFiles, otherFiles, genFiles, hasTestdata, protoFileInfo)
...
// returned.
func buildPackage(c *config.Config, dir, rel string, pkgFiles, otherFiles, genFiles []string, hasTestdata bool, protoInfo map[string]proto.FileInfo) *goPackage {
...
func (g *generator) generateProto(mode proto.Mode, pkg *goPackage) (string, []*rule.Rule) {
	...
	protoName := proto.RuleName(g.rel, pkg.name)
