...
func inferProtoMode(c *config.Config, rel string, f *rule.File) {
	...
	if pc.Mode != proto.DefaultMode || pc.ModeExplicit {
		return
	}
	if gc.prefix == wellKnownTypesGoPrefix {
		pc.Mode = proto.LegacyMode
		return
	}
	if path.Base(rel) == "vendor" {
		pc.Mode = proto.DisableMode
		return
	}
	if f == nil {
		return
...
outer:
	for _, l := range f.Loads {
		...
		if name == "@io_bazel_rules_go//proto:go_proto_library.bzl" {
			mode = proto.LegacyMode
			break
		}
		for _, sym := range l.Symbols() {
			if sym == "go_proto_library" {
				mode = proto.DisableMode
				break outer
			}
		}
	}
	if mode == proto.DefaultMode || pc.Mode == mode || c.ShouldFix && mode == proto.LegacyMode {
		return
	}
	pc.Mode = mode
}
