...
func (_ *goLang) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repos.RemoteCache, r *rule.Rule, from label.Label) {
	gc := getGoConfig(c)
	imports := r.PrivateAttr(config.GazelleImportsKey).(rule.PlatformStrings)
	r.DelAttr("deps")
	resolve := resolveGo
