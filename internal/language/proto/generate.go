...
func (_ *protoLang) GenerateRules(c *config.Config, dir, rel string, f *rule.File, subdirs, regularFiles, genFiles []string, other []*rule.Rule) (empty, gen []*rule.Rule) {
	...
	if pkg == nil {
		...
		empty = append(empty, rule.NewRule("proto_library", RuleName(rel, "")))
		return empty, gen
	}

	name := RuleName(rel, goPackageName(pkg))
...
// In the root directory, it returns root_proto.
// TODO(jayconrod): remove Go-specific functionality. This is here temporarily
// for compatibility.
func RuleName(rel, goPkgName string) string {
	if goPkgName != "" {
		return goPkgName + "_proto"
	}
	if base := path.Base(rel); base != "." {
		return base + "_proto"
	}
	return "root_proto"
}
